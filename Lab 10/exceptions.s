...
SERVICE_IRQ:
    			...
    			LDR		R5, [R4, #ICCIAR] 			// register stores ID of device that caused interrupt

PRIV_TIMER_CHECK:
    			CMP 	R5, #MPCORE_PRIV_TIMER_IRQ 	// Compares stored ID with timer interrupt ID 29 to see if this came from timer
 				BNE 	KEYS_CHECK 					// If not on timer, check if its key
 				BL 		PRIV_TIMER_ISR       //otherwise go to timer service routine
 				B 		EXIT_IRQ 
    
KEYS_CHECK:
    			CMP R5, #KEYS_IRQ                 // Compare ID at the I/O corresponding to KEYS (73)
UNEXPECTED: 	BNE UNEXPECTED                   //received an unexpected input
				BL KEY_ISR                      //otherwise go to key routine
